...
# ── Rate limiting delegation (async) ─────────────────────────────


async def test_rate_read_calls_limiter():
    client = BaseAPIClient(reads_per_sec=10, writes_per_sec=5)
    with patch.object(client._limiter, "acquire_read", new_callable=AsyncMock) as mock:
        await client._rate_read()
        mock.assert_awaited_once()


async def test_rate_write_calls_limiter():
    client = BaseAPIClient(reads_per_sec=10, writes_per_sec=5)
    with patch.object(client._limiter, "acquire_write", new_callable=AsyncMock) as mock:
        await client._rate_write()
        mock.assert_awaited_once()
